...
from app.authentication.service import now
...
def test_callback_rejects_invalid_identity(client, nonce, verified):
    ...
    with (
        patch("app.authentication.router.httpx.post") as post,
        patch("app.authentication.router.keys.get_signing_key_from_jwt"),
        patch("app.authentication.router.jwt.decode") as decode,
    ):
