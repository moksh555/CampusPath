...
from app.authentication.models import LoginSession, OAuthAttempt, User
from app.authentication.repository import upsert_google_user
from app.authentication.service import access_token, digest, issue_refresh, now, rotate
...
def login(db=Depends(get_db)):
    if (
        ...
        raise HTTPException(
            503, "Configure Google OAuth credentials in backend/auth_service/.env"
        )
    try:
        settings.signing_key()
    except ValueError as exc:
        raise HTTPException(
            503, "Configure JWT_SIGNING_KEY in backend/auth_service/.env"
        ) from exc
